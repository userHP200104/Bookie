...
const RequirementButton = (props) => {
  ...
  return (
      <>
      {isActive ? (
          <TouchableOpacity style={styles.requirementButtonActive} onPress={()=>navigation.navigate(props.route, {data: data})}>
            <MaterialCommunityIcons name={props.icon} color='#1a1a1a' size={26} />
            <Text style={styles.requirementTextActive}>{props.title}</Text>
          </TouchableOpacity> 
      ):(
          <TouchableOpacity style={styles.requirementButton} onPress={()=>navigation.navigate(props.route, {data: data})}>
